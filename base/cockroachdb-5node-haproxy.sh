...
# Generate the configuration file haproxy.cfg with the 5 node CRDB cluster
echo "Generate the configuration file haproxy.cfg with the 5 node CRDB cluster"
cockroach gen haproxy --insecure --host=localhost --port=26000

# Start the load balancer `HAProxy` using the configuration file `haproxy.cfg` and run it in the background with `&`
nohup haproxy -f haproxy.cfg > foo.out 2> foo.err < /dev/null & disown
